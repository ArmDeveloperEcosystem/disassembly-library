...
/*****************************************************************************
 * Everything below here is new stuff added for the Arm Disassembly Library. *
 *****************************************************************************/

//
// Create a disassembler for the TripleName. Every known feature set for the 
// architecture is added to the disassembler. Symbolic disassembly is supported
...
//
LLVMDisasmContextRef
LLVMCreateDisasmAllFeatures(const char *TT, void *DisInfo, 
                            int TagType, LLVMOpInfoCallback GetOpInfo,
                            LLVMSymbolLookupCallback SymbolLookUp) {
  ...
  // Add all known AArch64 extension feature sets
  std::string AllFeatureSets;
  if (TheTarget->getName() == std::string("aarch64")) {
    // Join all extension names into a comma separated list
    size_t ExtensionCount = sizeof(AArch64::Extensions) / sizeof(std::remove_all_extents<decltype(AArch64::Extensions)>::type);
    const size_t CommonMaxExtensionLength = 11;
    AllFeatureSets.reserve(CommonMaxExtensionLength * ExtensionCount);
    for (const auto& ExtensionInfo : AArch64::Extensions) {
      if (!ExtensionInfo.Feature.empty()) {
        AllFeatureSets.append(ExtensionInfo.Feature.data());
        AllFeatureSets.push_back(',');
      }
    }
    if (!AllFeatureSets.empty()) {
      AllFeatureSets.pop_back();
    }
  }

  return LLVMCreateDisasmCPUFeatures(TT, "", AllFeatureSets.c_str(), DisInfo, TagType, GetOpInfo, SymbolLookUp);
}

//
// LLVMDecodeAndDisasmInstruction() uses the disassembler context, specified in
// the parameter DCR, to decode and disassemble a single instruction specified
// by the parameter MIR. The bytes of the instruction are specified in the
// parameter Bytes, and contains at least BytesSize number of bytes. The
// instruction is at the address specified by the PC parameter. If a valid
// instruction can be disassembled its string is returned indirectly in
// OutString which whos size is specified in the parameter OutStringSize. This
// function returns the number of bytes in the instruction or zero if there was
// no valid instruction.
//
size_t LLVMDecodeAndDisasmInstruction(LLVMDisasmContextRef DCR,
                                      LLVMMCInstRef MIR, uint8_t *Bytes,
                                      uint64_t BytesSize, uint64_t PC,
                                      char *OutString, size_t OutStringSize) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  MCInst *Inst = static_cast<MCInst *>(MIR);
  // Wrap the pointer to the Bytes, BytesSize and PC in a MemoryObject.
  ArrayRef<uint8_t> Data(Bytes, BytesSize);
...
//
// Allocate a new instruction and return a reference to it.
//
LLVMMCInstRef LLVMMCInstCreate(void)
{
  MCInst *MI = new (std::nothrow) MCInst();
  LLVMMCInstRef MIR = static_cast<LLVMMCInstRef>(MI);
  return MIR;
}
...
//
void LLVMMCInstDispose(LLVMMCInstRef MIR)
{
  MCInst *MI = static_cast<MCInst *>(MIR);
  delete MI;
}
...
//
uint32_t LLVMMCInstGetNumOperands(LLVMMCInstRef MIR) {
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  unsigned int NumOperands = MI->getNumOperands();
  return static_cast<uint32_t>(NumOperands);
}

//
// Return the number of operands that are register definitions; the definitions
// always occur at the start of the operand list. This is the number of "outs"
// in the .td file, and does not include implicit defs.
//
uint32_t LLVMMCInstGetNumDefs(LLVMDisasmContextRef DCR, LLVMMCInstRef MIR) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  const MCInstrDesc &MID = DC->getInstrInfo()->get(MI->getOpcode());
  unsigned int NumDefs = MID.getNumDefs();
  return static_cast<uint32_t>(NumDefs);
}

//
// Return whether or not the instruction could possibly load memory.
//
uint32_t LLVMMCInstMayLoad(LLVMDisasmContextRef DCR, LLVMMCInstRef MIR) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  const MCInstrDesc &MID = DC->getInstrInfo()->get(MI->getOpcode());
  unsigned int MayLoad = MID.mayLoad();
  return static_cast<uint32_t>(MayLoad);
}

//
// Return whether or not the instruction could possibly modify memory.
//
uint32_t LLVMMCInstMayStore(LLVMDisasmContextRef DCR, LLVMMCInstRef MIR) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  const MCInstrDesc &MID = DC->getInstrInfo()->get(MI->getOpcode());
  unsigned int MayStore = MID.mayStore();
  return static_cast<uint32_t>(MayStore);
}

//
// Return the number of bytes in the encoding of this instruction.
//
uint32_t LLVMMCInstGetSize(LLVMDisasmContextRef DCR, LLVMMCInstRef MIR) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  const MCInstrDesc &MID = DC->getInstrInfo()->get(MI->getOpcode());
  unsigned int Size = MID.getSize();
  return static_cast<uint32_t>(Size);
}

//
// Return the mnemonic of the instruction opcode.
//
const char * LLVMMCInstGetOpcodeMnemonic(LLVMDisasmContextRef DCR, LLVMMCInstRef MIR) {
  LLVMDisasmContext *DC = static_cast<LLVMDisasmContext *>(DCR);
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  MCInstPrinter *MIP = DC->getIP();
  const char *Mnemonic = MIP->getMnemonic(MI).first;
  return Mnemonic;
}

//
// Return the number of register operands for the instruction.
//
uint32_t LLVMMCInstGetNumRegOperands(LLVMMCInstRef MIR) {
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  ...
  return static_cast<uint32_t>(NumReg);
}

//
// Return the number of immediate operands for the instruction.
//
uint32_t LLVMMCInstGetNumImmOperands(LLVMMCInstRef MIR) {
  const MCInst *MI = static_cast<const MCInst *>(MIR);
  unsigned int NumImm = 0;
  for (unsigned i = 0; i < MI->getNumOperands(); i++) {
    if (MI->getOperand(i).isImm()) {
      NumImm++;
    }
  }
  return static_cast<uint32_t>(NumImm);
}

/*****************************************************************************
 * Everything above here is new stuff added for the Arm Disassembly Library. *
 *****************************************************************************/
